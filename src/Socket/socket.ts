...
export const makeSocket = (config: SocketConfig) => {
	...
	const end = async (error: Error | undefined) => {
		...
		// emitting close, so consumers see all events that arrived
		...
		/**
		 * CONNECTION STABILITY: ev.destroy() clears all listeners, pending
		 * timers, history cache, and buffered data. Replaces the prior
		 * 'connection.update'-only removeAllListeners + destroy pair.
		 */
		ev.destroy()
	}
